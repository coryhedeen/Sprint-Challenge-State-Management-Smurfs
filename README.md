...
Actions govern any interaction with the website. Reducers compile the state and update the state
based on actions and the behavior of components. The store is essentially the state of the application but it is abstracted away from the application so that it is immutable. The store can
be thought of as the single source of truth because it is the only source of state.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the global state and Component state is the local state. If you don't want
to pass down data via props it would be good to use the Application state, but if you are working
with a very small amount of Component state it doesn't make sense to add the infrastructure
required to access global state.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Redux-thunk is a middleware that intercepts actions in order to deal with transitions in async
operations like loading. Thunk intercepts actions when they are called and ascertains whether they
are a function or an action. If they are a function the function will be invoked, if they are
an action then the action will be passed untouched.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
My favorite system is Redux because it makes the most sense. Having a store that bring state all the way down the prop line to a single component is a valuable tool. 
## Project Set Up
